...
rhs = sp.tan(0.0)  # This is a constant, tan(0) = 0

# Convert symbolic expressions to numerical functions
lhs_func = sp.lambdify((x, y), lhs, "numpy")
rhs_value = float(rhs)  # Convert RHS constant to float (0.0)
...
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_title(r'$ \cos(x) + \sin(y) \geq \tan(0) $ (2D Contour Plot)')
...
ax.set_title(r'$ \cos(x) + \sin(y) \geq \tan(0) $ (3D Surface Plot)')
